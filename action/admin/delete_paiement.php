...
try {
    ...
    // REQUETE SQL DELETE paiement
    $sql = "DELETE FROM public.paiements WHERE id = :id";
    ...
    // Suppression a affecté une ligne
    if ($stmt->rowCount() > 0) {
        $_SESSION['success_message'] = "Le paiement a été supprimé avec succès.";
    } else {
        $_SESSION['error_message'] = "Aucun paiement trouvé avec cet ID.";
    }

} catch (PDOException $e) {
    $_SESSION['error_message'] = "Erreur lors de la suppression du paiement : " . $e->getMessage();
} finally {
    $conn = null;
}
